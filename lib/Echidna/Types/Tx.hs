...
import EVM.Types (Expr(ConcreteBuf), Addr, W256)
...
getResult (VMSuccess (ConcreteBuf b))
  | b == encodeAbiValue (AbiBool True)  = ReturnTrue
  | b == encodeAbiValue (AbiBool False) = ReturnFalse
  | otherwise                           = Stop
